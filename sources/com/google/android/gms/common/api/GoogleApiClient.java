...
public abstract class GoogleApiClient {
    ...
    public static final class Builder {
        ...
        private GoogleApiClient zzarg() {
            ...
            Iterator<Api<?>> it = this.xL.keySet().iterator();
            Api<?> api3 = null;
            Api<?> api4 = null;
            while (true) {
                if (!it.hasNext()) {
                    if (api4 != null) {
                        ...
                        zzaa.zza(this.gj == null, "Must not set an account in GoogleApiClient.Builder when using %s. Set account in GoogleSignInOptions.Builder instead", api4.getName());
                        zzaa.zza(this.xF.equals(this.xG), "Must not set scopes in GoogleApiClient.Builder when using %s. Set account in GoogleSignInOptions.Builder instead.", api4.getName());
                    }
                ...
                Api<?> next = it.next();
                Api.ApiOptions apiOptions = this.xL.get(next);
                int i = zzavr.get(next) != null ? zzavr.get(next).DN ? 1 : 2 : 0;
                arrayMap.put(next, Integer.valueOf(i));
                zzqr zzqrVar = new zzqr(next, i);
                arrayList.add(zzqrVar);
                if (next.zzaqw()) {
                    Api.zzh<?, ?> zzaqu = next.zzaqu();
                    api2 = zzaqu.getPriority() == 1 ? next : api3;
                    api = next;
                    zza = zza(zzaqu, apiOptions, this.mContext, this.zzajy, zzarf, zzqrVar, zzqrVar);
                } else {
                    api = next;
                    Api.zza<?, ?> zzaqt = api.zzaqt();
                    api2 = zzaqt.getPriority() == 1 ? api : api3;
                ...
                arrayMap2.put(api.zzaqv(), zza);
                if (zza.zzajc()) {
                    if (api4 != null) {
                        String valueOf3 = String.valueOf(api.getName());
                        String valueOf4 = String.valueOf(api4.getName());
                        StringBuilder sb2 = new StringBuilder(21 + String.valueOf(valueOf3).length() + String.valueOf(valueOf4).length());
                        sb2.append(valueOf3);
