...
public class UsageInfo extends AbstractSafeParcelable {
    ...
    private static DocumentSection zzk(Uri uri) {
        return new DocumentSection(uri.toString(), new RegisterSectionInfo.zza("web_url").zzco(4).zzaz(true).zzfs(FileDownloadModel.URL).zzahr());
    }
    ...
    private static DocumentSection zzv(List<AppIndexApi.AppIndexingLink> list) {
        ...
        zzvw.zza.C0040zza[] c0040zzaArr = new zzvw.zza.C0040zza[list.size()];
        for (int i = 0; i < c0040zzaArr.length; i++) {
            c0040zzaArr[i] = new zzvw.zza.C0040zza();
            AppIndexApi.AppIndexingLink appIndexingLink = list.get(i);
            c0040zzaArr[i].ahD = appIndexingLink.appIndexingUrl.toString();
            c0040zzaArr[i].viewId = appIndexingLink.viewId;
            if (appIndexingLink.webUrl != null) {
                c0040zzaArr[i].ahE = appIndexingLink.webUrl.toString();
            }
        }
        zzaVar.ahB = c0040zzaArr;
