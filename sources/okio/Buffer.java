...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable, ByteChannel {
    ...
    public static final class UnsafeCursor implements Closeable {
        ...
        public int seek(long j) {
            ...
            if (j3 - j > j - j2) {
                while (j >= j2 + (segment.limit - segment.pos)) {
                    long j5 = j2 + (segment.limit - segment.pos);
    ...
    public byte getByte(long j) {
        ...
        if (this.size - j > j) {
            ...
            while (true) {
                long j4 = j3 + (segment2.limit - segment2.pos);
                if (j4 >= 0) {
    ...
    public long indexOf(byte b, long j, long j2) {
        ...
        if (this.size - j >= j) {
            while (true) {
                long j5 = j3 + (segment.limit - segment.pos);
                if (j5 >= j) {
                    break;
                }
                segment = segment.next;
        ...
        while (j3 < j4) {
            ...
            for (int i = (int) ((segment.pos + j6) - j3); i < min; i++) {
                if (bArr[i] == b) {
                    return (i - segment.pos) + j3;
                }
            }
            long j7 = j3 + (segment.limit - segment.pos);
            segment = segment.next;
            j6 = j7;
            j3 = j6;
        }
    ...
    public long indexOf(ByteString byteString, long j) throws IOException {
        ...
        if (buffer.size - j >= j) {
            while (true) {
                long j4 = j2 + (segment2.limit - segment2.pos);
                if (j4 >= j) {
                    break;
                }
                segment2 = segment2.next;
    ...
    public long indexOfElement(ByteString byteString, long j) {
        ...
        if (byteString.size() == 2) {
            ...
            while (j2 < this.size) {
                ...
                long j4 = j2 + (segment.limit - segment.pos);
                segment = segment.next;
                j = j4;
                j2 = j;
            }
        ...
        return new InputStream() { // from class: okio.Buffer.2
            ...
            public int read() {
                if (Buffer.this.size > 0) {
                    return Buffer.this.readByte() & FileDownloadStatus.error;
                }
                return -1;
            }
    ...
    public boolean rangeEquals(long j, ByteString byteString, int i, int i2) {
        ...
        for (int i3 = 0; i3 < i2; i3++) {
            if (getByte(j + i3) != byteString.getByte(i + i3)) {
                return false;
            }
        }
        return true;
    ...
    /* JADX WARN: Removed duplicated region for block: B:49:0x00b8 A[EDGE_INSN: B:49:0x00b8->B:43:0x00b8 BREAK  A[LOOP:0: B:7:0x0018->B:48:?], SYNTHETIC] */
