...
/* loaded from: apklis.apk:jacocoagent.jar:org/jacoco/agent/rt/internal_b0d6a23/asm/ClassReader.class */
public class ClassReader {
    ...
    public ClassReader(String name) throws IOException {
        this(readClass(ClassLoader.getSystemResourceAsStream(name.replace('.', '/') + ClassUtils.CLASS_FILE_SUFFIX), true));
    }
    ...
    */
    private static byte[] readClass(java.io.InputStream r6, boolean r7) throws java.io.IOException {
        ...
        L9f:
            ...
            throw r0
        */
        throw new UnsupportedOperationException("Method not decompiled: org.jacoco.agent.rt.internal_b0d6a23.asm.ClassReader.readClass(java.io.InputStream, boolean):byte[]");
    }
    ...
    private int readMethod(ClassVisitor classVisitor, Context context, int u) {
        ...
        if (methodParameters != 0) {
            int i2 = this.b[methodParameters] & FileDownloadStatus.error;
    ...
    private void readCode(MethodVisitor mv, Context context, int u) {
        ...
        if (stackMap != 0) {
            ...
            for (int i6 = stackMap; i6 < (stackMap + stackMapSize) - 2; i6++) {
                if (b[i6] == 8 && (v = readUnsignedShort(i6 + 1)) >= 0 && v < codeLength && (b[u2 + v] & FileDownloadStatus.error) == 187) {
                    readLabel(v, labels);
                }
            }
        }
        ...
        while (u6 < codeEnd) {
            ...
            int opcode = b[u6] & FileDownloadStatus.error;
            switch (ClassWriter.TYPE[opcode]) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    mv.visitVarInsn(opcode, b[u6 + 1] & FileDownloadStatus.error);
                    u6 += 2;
                    break;
                case 4:
                    if (opcode > 54) {
                ...
                case 10:
                    ...
                    break;
                case 11:
                    mv.visitLdcInsn(readConst(b[u6 + 1] & FileDownloadStatus.error, c));
                    u6 += 2;
                    break;
                case 12:
                    ...
                    break;
                case 13:
                    mv.visitIincInsn(b[u6 + 1] & FileDownloadStatus.error, b[u6 + 2]);
                    u6 += 3;
                    break;
                case 14:
                ...
                case 16:
                default:
                    mv.visitMultiANewArrayInsn(readClass(u6 + 1, c), b[u6 + 3] & FileDownloadStatus.error);
                    u6 += 4;
                    break;
                case 17:
                    int opcode4 = b[u6 + 1] & FileDownloadStatus.error;
                    if (opcode4 == 132) {
            ...
            for (int i13 = readUnsignedShort(varTable); i13 > 0; i13--) {
                ...
                if (typeTable != null) {
                    ...
                    while (true) {
                        ...
                        if (typeTable[j4] != start2 || typeTable[j4 + 1] != index) {
                            j4 += 3;
                        } else {
    ...
    private void readParameterAnnotations(MethodVisitor mv, Context context, int v, boolean visible) {
        int v2 = v + 1;
        int n = this.b[v] & FileDownloadStatus.error;
    ...
    private int readAnnotationValue(int v, char[] buf, String name, AnnotationVisitor av) {
        if (av == null) {
            switch (this.b[v] & FileDownloadStatus.error) {
                case 64:
        ...
        switch (this.b[v] & FileDownloadStatus.error) {
            ...
            case 91:
                ...
                int v4 = v3 + 1;
                switch (this.b[v3] & FileDownloadStatus.error) {
                    case 66:
    ...
    /*
        ...
        To view partially-correct add '--show-bad-code' argument
    */
    private void getImplicitFrame(org.jacoco.agent.rt.internal_b0d6a23.asm.Context r7) {
        /*
            Method dump skipped, instructions count: 378
            To view this dump add '--comments-level debug' option
        */
        throw new UnsupportedOperationException("Method not decompiled: org.jacoco.agent.rt.internal_b0d6a23.asm.ClassReader.getImplicitFrame(org.jacoco.agent.rt.internal_b0d6a23.asm.Context):void");
    }
    ...
    private int readFrame(int stackMap, boolean zip, boolean unzip, Context frame) {
        ...
        if (zip) {
            stackMap++;
            tag = this.b[stackMap] & FileDownloadStatus.error;
        } else {
            tag = 255;
            frame.offset = -1;
        }
    ...
    public int readByte(int index) {
        return this.b[index] & FileDownloadStatus.error;
    }
    ...
    private String readUTF(int index, int utfLen, char[] buf) {
        ...
        while (index < endIndex) {
            ...
            switch (st) {
                case 0:
                    int c = b2 & FileDownloadStatus.error;
                    if (c < 128) {
                        int i2 = strLen;
                        strLen++;
                        buf[i2] = (char) c;
