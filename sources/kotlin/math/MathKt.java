...
public final class MathKt {
    ...
    public static final double asinh(double d) {
        if (d < taylor_n_bound) {
            return d <= (-taylor_n_bound) ? -asinh(-d) : Math.abs(d) >= taylor_2_bound ? d - (((d * d) * d) / 6) : d;
        }
    ...
    public static final double atanh(double d) {
        if (Math.abs(d) < taylor_n_bound) {
            return Math.abs(d) > taylor_2_bound ? d + (((d * d) * d) / 3) : d;
        }
        double d2 = 1;
